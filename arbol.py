...
def tree_graph(stack):
    ...
    num = 0
    for nodo in stack:
        node = pydotplus.Node(str(nodo.id))  # Use el ID único del nodo como identificador en el gráfico
    ...
    for nodo in stack:
        if nodo.padre is not None and nodo.padre in state_nodes:
            if nodo.id in state_nodes:
